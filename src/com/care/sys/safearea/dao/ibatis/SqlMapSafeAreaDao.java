...
public class SqlMapSafeAreaDao extends SqlMapClientDaoSupport implements SafeAreaDao{
	...
	@SuppressWarnings("unchecked")
	public List<DataMap> getWeFencingInfo(SafeArea vo)
			throws DataAccessException {
		logger.debug("getSafeAreaListByVo(SafeArea vo)");
		return getSqlMapClientTemplate().queryForList("getWeFencingInfo", vo);
	}
	public int getgetWeFencingInfoCount(SafeArea vo) throws DataAccessException {
		logger.debug("getgetWeFencingInfoCount(SafeArea vo)");
		return (Integer)getSqlMapClientTemplate().queryForObject("getgetWeFencingInfoCount", vo);
